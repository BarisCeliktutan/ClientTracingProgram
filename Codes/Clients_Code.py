...
class Clients(QWidget):
    def __init__(self):
        ...
        self.clients_win.tbwIslemTuru.setColumnHidden(0, True)  # id

        self.clients_win.tbwHareketler.setColumnHidden(0, True)  # id
        self.clients_win.tbwHareketler.setColumnHidden(5, True)  # musteri_id
        self.clients_win.tbwHareketler.setColumnHidden(6, True)  # islem_turu_id
        self.clients_win.tbwHareketler.setColumnHidden(7, True)  # islem_tipi_id
    ...
    def islem_turleri(self):
        ...
        for i in range(len(islem_turleri)):
            ...
            item = self.clients_win.tbwIslemTuru.item(i, 1)  # 'item = ...' şeklinde yazmamın nedeni text'i ortalamak!
    ...
    def secildi(self, row):
        ...
        # self.bilgileri(row)
        # deger = self.clients_win.tbwMusteriler.cellWidget(row, 0).isChecked()
        # degistir_query = f"UPDATE musteriler SET AKTIF = {deger} WHERE ID={self.clients_win.tbwMusteriler.item(row, 10).text()};"
        # Server(degistir_query, "işle").server_baglanti()
    ...
    def hareket_tablosu(self, hareket_query):
        # hareket_query = f"SELECT * FROM view_hareketler WHERE SILINDI = 0"
